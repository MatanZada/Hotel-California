$(function () {
    ...
    $('[data-role="save-btnAllReservation"]').click(function () {
        $.ajax({
            ...
            success: function (result) {
                console.log(result);
                document.getElementById("getAllReservation").textContent = JSON.stringify(result, undefined, 2);
            }
        });
    })
    ...
    $('[data-role="save-btnFindOneReservation"]').click(function () {
        ...
        $.get({
            ...
            success: function (result) {
                document.getElementById("findOneReservation").textContent = JSON.stringify(result);
            },
        });
    })

    $('[data-role="save-btnFindOneReservationAndUpdate"]').click(function () {
        let reservationFindOneAndUpdate = $('[data-role="findOneReservationAndUpdate"]').val()
        $.ajax({
            ...
            success: function (result) {
                document.getElementById("findOneReservationAndUpdate").textContent = JSON.stringify(result, undefined, 2);
            }
        });
    })

    $('[data-role="DeleteOnereservationById"]').click(function () {
        let oneReservationDelete = $('[data-role="DeleteOnereservationById"]').val()
        $.ajax({
